...
from config import DISTILLED_OUTPUT_DIR_S, DATASET_NAME, MAX_LENGTH, TEACHER_MODEL
...
def main():
    print(f"Loading Student model from: {DISTILLED_OUTPUT_DIR_S}")
    tokenizer = AutoTokenizer.from_pretrained(DISTILLED_OUTPUT_DIR_S)
    student = AutoModelForCausalLM.from_pretrained(DISTILLED_OUTPUT_DIR_S, torch_dtype=torch.float32, device_map="auto")
